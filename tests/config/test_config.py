...
def test_pg_settings(fake_env: None) -> None:
    pg_settings = PostgresSettings()
    assert (
        pg_settings.postgres_dsn
        == "postgresql+asyncpg://test_user:test_pwd@localhost:5439/test_db"
    )


def test_redis_settings(fake_env: None) -> None:
    redis_settings = RedisSettings()
    assert redis_settings.redis_dsn == "redis://:test_pwd@localhost:6380/0"


def test_jwt_settings(fake_env: None) -> None:
...
def test_pwd_settings(fake_env: None) -> None:
...
def test_aws_settings(fake_env: None) -> None:
    aws_settings = AWSSettings()
    assert aws_settings.AWS_ENDPOINT == "http://localhost:4566"
    assert aws_settings.AWS_DEFAULT_REGION == "default"
    assert (
        aws_settings.AWS_SECRET_ACCESS_KEY.get_secret_value() == "test_secret"
    )
